...
def parse_args():
    ...
    parser.add_argument("--mad_data_path", type=str,
                        default='/nfs/data3/user/revisionllm/activitynet/train.json')
    ...
    parser.add_argument("--mad_out_path", type=str,
                        default="/nfs/data3/user/revisionllm/activitynet_grounding_train.json")
...
def convert_mad_to_vtimellm(mad_data):
    ...
    for key, value in mad_data.items():
        for sentence_id, (timestamps, sentence) in enumerate(zip(value['timestamps'], value['sentences'])):
            ...
            second_dict['meta'] = {'duration': value["duration"],
                                   'token': token}
